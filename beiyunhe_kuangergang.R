...
###################C
C_chushi <- read_excel("M:/myworkspace/COD_beiyunhe/kuangergang.xlsx", range = "B1:B169", sheet = 'Sheet1')
C_chushi = C_chushi[[1]]
library(fitdistrplus)
fitdist(C_chushi,'gamma')

sample_C = rgamma(50,shape=4.0666892, rate=0.2115652)    ####抽取多少???
sample_C = rgamma(50,shape=1.111398, rate=2.802787)   #NH3N
sample_C = rgamma(50,shape=1.250317, rate=2.733033)   #TP
...
Cs = 40
W = seq(110,310,by=1)
W = seq(1.8,5,by=0.01)  #NH3N
W = seq(0.9,3.36,by=0.01)  #TP


p_W_prior = dnorm(W,mean=213.4,sd=47)
p_W_prior = dnorm(W,mean=3.4,sd=0.8)   #NH3N
p_W_prior = dnorm(W,mean=2.13,sd=0.7)   #TP
